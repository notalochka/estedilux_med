import { Event } from '@/types/events';

export const events: Event[] = [
  {
    id: 1,
    categoryId: 2,
    title: {
      ru: 'Пошаговый 3-й курс по Ринопластике Cadaver Course и живая хирургия',
      en: 'Step-by-step 3rd Rhinoplasty Cadaver Course and Live Surgery',
      tr: 'Adım Adım 3. Rinoplasti Kadavra Kursu ve Canlı Cerrahi',
      uk: 'Покроковий 3-й курс з Ринопластики Cadaver Course та жива хірургія',
    },
    description: {
      ru: 'Практический курс по ринопластике с использованием кадаверов и живой хирургии. Изучение анатомии носа и современных техник ринопластики.',
      en: 'Practical rhinoplasty course using cadavers and live surgery. Study of nasal anatomy and modern rhinoplasty techniques.',
      tr: 'Kadavralar ve canlı cerrahi kullanılarak pratik rinoplasti kursu. Burun anatomisi ve modern rinoplasti tekniklerinin incelenmesi.',
      uk: 'Практичний курс з ринопластики з використанням кадаверів та живої хірургії. Вивчення анатомії носа та сучасних технік ринопластики.',
    },
    date: '2026-01-15',
    location: {
      ru: 'Стамбул, Турция',
      en: 'Istanbul, Turkey',
      tr: 'İstanbul, Türkiye',
      uk: 'Стамбул, Туреччина',
    },
    image: '/events/1.jpg',
    price: 2500,
  },
  {
    id: 2,
    categoryId: 2,
    title: {
      ru: 'Пошаговый 3-й курс по Ринопластике Cadaver Course и живая хирургия',
      en: 'Step-by-step 3rd Rhinoplasty Cadaver Course and Live Surgery',
      tr: 'Adım Adım 3. Rinoplasti Kadavra Kursu ve Canlı Cerrahi',
      uk: 'Покроковий 3-й курс з Ринопластики Cadaver Course та жива хірургія',
    },
    description: {
      ru: 'Практический курс по ринопластике с использованием кадаверов и живой хирургии. Изучение анатомии носа и современных техник ринопластики.',
      en: 'Practical rhinoplasty course using cadavers and live surgery. Study of nasal anatomy and modern rhinoplasty techniques.',
      tr: 'Kadavralar ve canlı cerrahi kullanılarak pratik rinoplasti kursu. Burun anatomisi ve modern rinoplasti tekniklerinin incelenmesi.',
      uk: 'Практичний курс з ринопластики з використанням кадаверів та живої хірургії. Вивчення анатомії носа та сучасних технік ринопластики.',
    },
    date: '2026-09-15',
    location: {
      ru: 'Стамбул, Турция',
      en: 'Istanbul, Turkey',
      tr: 'İstanbul, Türkiye',
      uk: 'Стамбул, Туреччина',
    },
    image: '/events/2.jpg',
    price: 2800,
  },
  {
    id: 3,
    categoryId: 1,
    title: {
      ru: 'Пошаговый 3-й курс по Ринопластике Cadaver Course и живая хирургия',
      en: 'Step-by-step 3rd Rhinoplasty Cadaver Course and Live Surgery',
      tr: 'Adım Adım 3. Rinoplasti Kadavra Kursu ve Canlı Cerrahi',
      uk: 'Покроковий 3-й курс з Ринопластики Cadaver Course та жива хірургія',
    },
    description: {
      ru: 'Практический курс по ринопластике с использованием кадаверов и живой хирургии. Изучение анатомии носа и современных техник ринопластики.',
      en: 'Practical rhinoplasty course using cadavers and live surgery. Study of nasal anatomy and modern rhinoplasty techniques.',
      tr: 'Kadavralar ve canlı cerrahi kullanılarak pratik rinoplasti kursu. Burun anatomisi ve modern rinoplasti tekniklerinin incelenmesi.',
      uk: 'Практичний курс з ринопластики з використанням кадаверів та живої хірургії. Вивчення анатомії носа та сучасних технік ринопластики.',
    },
    date: '2026-08-15',
    location: {
      ru: 'Стамбул, Турция',
      en: 'Istanbul, Turkey',
      tr: 'İstanbul, Türkiye',
      uk: 'Стамбул, Туреччина',
    },
    image: '/events/3.jpg',
    price: 3000,
  },
];

